...
Page({
  ...
  onLoad: async function (_options) {
    ...
    await this.init_charList()
    this.setData({
      openid: app.openid,
      roomlist: app.roomlist
    })
    // console.log("输出app.roomlist", app.roomlist)
  ...
  */
  async init_charList() {
    ...
    const db = wx.cloud.database();
    // console.log("enter A");
    const { data: datas } = await db.collection('rooms').where({
      p_s: myid,
      deleted: 0
    }).get()
    ...
    if (datas.length > 0) {
      for (var i = 0; i < datas.length; i++) {
        var dia = new Object();
        dia.roomid = datas[i]._id;
        dia.openid = datas[i].p_b;
        dia.time = "";
        dia.cha = "买家:";
        dia.name = "";
        dia.image = "";
    ...
    const { data: datab } = await db.collection('rooms').where({
      p_b: myid,
      deleted: 0
    }).get()
    // console.log(res.data);
    if (datab.length > 0) {
      for (var i = 0; i < datab.length; i++) {
        var dia = new Object();
        dia.roomid = datab[i]._id;
        dia.openid = datab[i].p_s;
        dia.time = "";
        dia.cha = "卖家:";
        dia.name = "";
        dia.image = "";

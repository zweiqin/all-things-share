...
Page({
  ...
  detail(e) {
    let that = this;
    wx.navigateTo({
      url: '/pages/detail/detail?scene=' + e.currentTarget.dataset.id, //2.详情页
    })
  },
  //获取轮播
  getbanner() {
  ...
  //跳转轮播链接
  goweb(e) {
    if (e.currentTarget.dataset.web.url == '1') {
      wx.navigateTo({ //3.轮播页
        url: '/pages/web/web?url=1',//数据库的banner表下面的第一项（对象），里面的list数组里面的某一个item项（对象）（即传过来的web），里面的url键的值
      })
    } else if (e.currentTarget.dataset.web.url == '2') {
      wx.navigateTo({
        url: '/pages/web/web?url=2',
      })
    }
  },
  onShareAppMessage() {
